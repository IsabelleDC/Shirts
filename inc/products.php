...
/*
 * Look for a search term in the product names
...
* return a boolean false if no product matches the sku
...
function get_product_single($sku){
    ...
    try{
        // create a pdo statement object with a query with a ? in place of a sku
        $results = $db->prepare("SELECT name, price, img, sku, paypal FROM products WHERE sku = ?");
        // bind the sku variable
        $results->bindParam(1,$sku);
        $results->execute();
    } catch (Exception $e) {
        echo "Daya could not be retrieved from db";
        exit;
    }
    // call the fetch method to retrieve the product information
    ...
    try{
        ...
        echo "Data could not be retieved from the db";
        exit;
    }
